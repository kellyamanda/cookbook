...
cookbook = st.sidebar.radio('Cookbook',['Description', 'Dataframe formatting', 'Side-by-side charts', 'Select rows', 'Range slider', 'Advanced selectbox', 'Seaborn', 'Wordcloud', 'Emojis',])
...
if cookbook == 'Side-by-side charts':
    ...
    '''
    ## Side-by-side charts
    Streamlit will support horizontal layout by late 2020. In the meantime, you can use Altair and Plotly for side-by-sidebar
    layout of charts.
    ...
    with st.echo():
        ...
        side_by_side_chart = alt.Chart(df).mark_bar().encode(
            x='animal',
            y='count',
        ).properties(
            width=200,
    ...
    with st.echo():
        ...
        # Simple scatter plot
        scatter = go.Line(x = [5, 20, 35],
                            y = [30, 20, 10]
        )

        # Simple bar chart
        bar = go.Bar(x=['velociraptor_2010', 'velociraptor_2011', 'velociraptor_2012', 'velociraptor_2013'],
                        y=[5, 8, 11, 14],
...
if cookbook == 'Select rows':
    ...
    df = pd.DataFrame({'date': ['2019-08-01', '2019-08-01', '2019-08-02', '2019-08-02'],
...
if cookbook == 'Emojis':
    '''
    ## Emojis in Streamlit
    Just use markdown formatting! Streamlit supports most standard emojis. [See full list here](https://raw.githubusercontent.com/omnidan/node-emoji/master/lib/emoji.json).
    '''
